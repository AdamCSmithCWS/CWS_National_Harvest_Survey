...
for(g in 1:ngroups){
  ...
  for(g in 1:ngroups){
    ...
    alpha_psi[g] ~ dgamma(2,0.7) # this prior generates an almost uniform prior on psi - with more mass at 0 and 1
    beta_psi[g] ~ dgamma(2,0.7)
    psi[g] ~ dbeta(alpha_psi[g],beta_psi[g])T(0.0001,0.9999) # avoids infinite probabilites at 0 and 1
