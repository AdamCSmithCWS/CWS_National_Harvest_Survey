...
##################################################################################
  ...
  for(y in 1:nyears){
    w_sy[1:nspecies,y] ~ dmulti(pcomp_sy[1:nspecies,y], nparts_y[y])   # multinom distr vector responses
  ...
  for (s in 1:nspecies){
    for (y in 1:nyears){
      ...
      delta_sy[s,y] ~ dgamma(exp_alpha_sy[s,y], 1)
  ...
  for(s in 2:nspecies){
    ...
    for(y in 2:nyears){
      alpha_sy[s,y] ~ dnorm(alpha_sy[s,y-1],tau_alpha_s[s]) 
    }
  }#s
  ...
  for(s in 1:nspecies){
    ...
    tau_alpha_s[s] ~ dscaled.gamma(0.02,5) #year-effecct variance by species
    taualpha_perod_s[s] ~ dscaled.gamma(0.5,50) # period variance by species
    sd_alpha_s[s] <- 1/sqrt(tau_alpha_s[s])
    sd_alpha_period_s[s] <- 1/sqrt(taualpha_perod_s[s])
    
    alpha_ps[1,s] <- 0
    for(y in 1:nyears){
      #alpha_psy1[p,s,y] ~ dnorm(0,tau_alpha_psy[s]) 
      alpha_psy[1,s,y] <- alpha_sy[s,y] + alpha_ps[1,s]
      
    }
    for(p in 2:nperiods){
       alpha_ps[p,s] ~ dnorm(0,taualpha_perod_s[s]) #species/period-specific effect
