...
##################################################################################
 ...
 #   }#s   
  ...
  alpha_s[1] <- 0
  tau_alpha_ps[1] ~ dscaled.gamma(0.1,50)
  
  # species hyperparameters alpha_s
  for(s in 2:nspecies){
    alpha_s[s] ~ dnorm(0,0.1)
    tau_alpha_ps[s] ~ dscaled.gamma(0.1,50) # species variance among periods
  }
  
  # species by period mid-year intercepts alpha_ps
  for(p in 1:nperiods){
    alpha_ps[p,1] <- 0
    tau_alpha_psy[p,1] ~ dscaled.gamma(0.1,50) 
    #### multinomial log-ratios, time-series model for the species composition
  for(s in 2:nspecies){
    alpha_ps[p,s] ~ dnorm(alpha_s[s],tau_alpha_ps[s])
    tau_alpha_psy[p,s] ~ dscaled.gamma(0.1,50) # species by period variance through time
  }
  
  ### first species is the reference group in each year and period
  for(y in 1:nyears){
    alpha_psy[p,1,y] <- 0 #~ dnorm(alpha_sy[1,y-1],tau_alpha_s[s]) 
  }
  
  for(s in 2:nspecies){
    alpha_psy[p,s,midyear] <- alpha_ps[p,s] # ~ dnorm(0,0.5) 
    
    for(y in (midyear+1):nyears){
      
      alpha_psy[p,s,y] ~ dnorm(alpha_psy[p,s,y-1],tau_alpha_psy[p,s]) 
    }
    for(y in 1:(midyear-1)){
      alpha_psy[p,s,y] ~ dnorm(alpha_psy[p,s,y+1],tau_alpha_psy[p,s]) 
    }
